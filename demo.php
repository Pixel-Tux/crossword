...
<form>
	Columns: <input type="text" name="cols" value="<?php echo $pc->cols; ?>" size="5" />
	Rows: <input type="text" name="rows" value="<?php echo $pc->rows; ?>" size="5" />
	Words: <input type="text" name="max_words" value="<?php echo $pc->max_words; ?>" size="5" />
	
	Group: 
	<select name="groupid">
	<?php foreach ($pc->getGroupIDs() as $groupid): ?>
	<option value="<?php echo $groupid; ?>" <?php if ($groupid == $pc->groupid) echo 'selected'; ?>>
		<?php echo $groupid?> [w: <?php echo $pc->countWordsInGroup($groupid);?>]
	</option>
	<?php endforeach; ?>
	</select>
	
	Debug:
	<input type="checkbox" name="colors" value="1" <?php if (!empty($_REQUEST['colors'])) echo 'checked'; ?> />

	Big blocks?:
	<input type="checkbox" name="bigblocks" value="1" <?php if (!empty($_REQUEST['bigblocks'])) echo 'checked'; ?> />
...
<!--	<td><?php echo $word["x"]?></td>
	<td><?php echo $word["y"]?></td>
	<td><?php echo $word["axis"]?></td> -->
</tr>
<?php endforeach; ?>
</table>
